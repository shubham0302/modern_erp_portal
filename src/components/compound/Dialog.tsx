...
const Dialog: React.FC<DialogProps> = (props) => {
  ...
  const defaultContent = (
    <>
      <div className={cn("flex shrink-0 items-center gap-4", paddingClass)}>
        <div className="mr-auto flex flex-col">
          {title && (
            <h5 className="text-nl-700 dark:text-nd-50 font-medium">{title}</h5>
          )}
          {subTitle && (
            <p className="text-nl-400 dark:text-nd-300 mt-0.5">{subTitle}</p>
          )}
        </div>
        {trialingTitleComponent && trialingTitleComponent}
      ...
      {actions && (
        <div className={cn("flex shrink-0 justify-end gap-x-2", paddingClass)}>
          {actions?.tertiary && (
            <Button
  ...
  const dialogContent = (
    ...
    >
      ...
      >
        {shouldRenderContent && (customContent ? children : defaultContent)}
      </div>
    </div>
  );
